...
class BidServiceTest {
    ...
    fun `should add first bid with starting price`() {
        ...
        bidService.addBid(auctionItemId, "test@example.com", 10, "").shouldBeRight()
    ...
    fun `should return error when last bid id is empty but bids exist`() {
        ...
        bidService.addBid(auctionItemId, "test@example.com", 10, "").apply {
            shouldBeLeft()
    ...
    fun `should return error when last bid id is not the last bid id`() {
        ...
        bidService.addBid(auctionItemId, "test@example.com", 10, lastBidId).apply {
            shouldBeLeft()
    ...
    fun `should add bid successfully`() {
        ...
        bidService.addBid(auctionItemId, "test@example.com", 10, lastBidId).shouldBeRight()
        bidDao.findByAuctionItemId(auctionItemId).apply {
            shouldHaveAtLeastSize(6)
            last().bidPrice.shouldBe(10.00)
        }
    }
